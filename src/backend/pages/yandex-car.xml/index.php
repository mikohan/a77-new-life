...
foreach ($products['hits']['hits'] as $product_source) {
  ...
  $name = mb_ucfirst($product['name']) . ' на ' . $model . ', ' . $brand . ', ' . $cat_number  . ' ' . $name2;
  ...
  $offers .= <<<HTML
    <offer id="{$id}">
      ...
      <vendor>{$brand}</vendor>
      <description>{$name} на {$make} {$model} в наличии и на заказ . Подберем оригинал и аналоги. Пришлем видео по запросу.</description>
      <name>{$name}</name>
